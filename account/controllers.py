...
def get_user_list(page_num=None, page_size=None):
    ...
    获取用户列表
    '''
    total = UserModel.objects.filter(is_deleted=False).exclude(username='root').count()
    if page_num and page_size:
        end = page_num * page_size
        start = end - page_size
        objs = UserModel.objects.filter(is_deleted=False).exclude(username='root').all()[start:end]
    else:
        objs = UserModel.objects.filter(is_deleted=False).exclude(username='root').all()
...
def get_role_list(page_num=None, page_size=None):
    ...
    获取角色列表
    '''
    total = RoleModel.objects.filter(is_deleted=False).exclude(sign='root').count()
    if page_num and page_size:
        end = page_num * page_size
        start = end - page_size
        objs = RoleModel.objects.filter(is_deleted=False).exclude(sign='root').all()[start:end]
    else:
        objs = RoleModel.objects.filter(is_deleted=False).exclude(sign='root').all()
    data_list = [obj.to_dict() for obj in objs]
    data = {
        'total': total,
        'data_list': data_list,
...
def get_mod_list(page_num=None, page_size=None):
    ...
    获取模块列表
    '''
    total = ModModel.objects.filter(is_deleted=False).count()
    if page_num and page_size:
        end = page_num * page_size
        start = end - page_size
        objs = ModModel.objects.filter(is_deleted=False).all()[start:end]
    else:
        objs = ModModel.objects.filter(is_deleted=False).all()
    data_list = [obj.to_dict() for obj in objs]
    data = {
        'total': total,
        'data_list': data_list,
...
def get_url_list(page_num=None, page_size=None):
    ...
    获取URL列表
    '''
    total = UrlModel.objects.filter(is_deleted=False).count()
    if page_num and page_size:
        end = page_num * page_size
        start = end - page_size
        objs = UrlModel.objects.filter(is_deleted=False).all()[start:end]
    else:
        objs = UrlModel.objects.filter(is_deleted=False).all()
    data_list = [obj.to_dict() for obj in objs]
    data = {
        'total': total,
        'data_list': data_list,
